...
const ZapCard: React.FC<ZapCardProps> = ({ swap = Swap.QuickSwap }) => {
  ...
  const [
    ...
    subTokenOutBalance0,
    subTokenOutBalance1
  ] = useZapTokenBalance(tokenOut, swap);
  const [convertedTokenCount, tradeException] = useTrades(
    tokenIn,
    tokenOut,
    toBalanceFromDisplay(val, tokenInDecimal),
    swap
  );
  ...
  useEffect(() => {
    ...
    const defaultTokenOut = zapItems.find(
      (zap) => zap.name === (swap === Swap.QuickSwap ? "polyBUNNY" : "ETH")
    );
    const defaultTokenDecimal = zapItems.find((zap) => zap.name === "MATIC")
      .decimals[0];
    ...
    if (queryMap.out) {
      const queryTokenOut = zapItems.find(
        (zap) => zap.address === queryMap.out
      );
      if (queryTokenOut) {
  ...
  const executeZap = async () => {
    ...
    firebase
      .analytics()
      .logEvent("zap", { in: tokenIn.name, out: tokenOut.name, value: val });
  ...
  const getTokenOutBalanceView = (): ReactNode => {
    if (!tokenOut)
      return <span className="value">{toDisplayBalanceComma(ZERO)}</span>;

    if (tokenOut.virtual) {
      return (
        <span className="value">
          {toDisplayBalanceComma(subTokenOutBalance0, tokenOut.decimals[0])}{" "}
          {tokenOut.symbols[0]} +{" "}
          {toDisplayBalanceComma(subTokenOutBalance1, tokenOut.decimals[1])}{" "}
          {tokenOut.symbols[1]}
        </span>
      );
    } else {
      return (
        <span className="value">
          {toDisplayBalanceComma(tokenOutBalance, tokenOut.decimals[0])}{" "}
          {tokenOut.name}
        </span>
      );
    }
  ...
  const getActionView = (): ReactNode => {
    ...
    return (
      <CardContent>
        <div className="zap-control-action-wrapper">{getActionButton()}</div>
      </CardContent>
    );
  };
  ...
  return (
    <div className="zap-wrapper">
      <div className="zap-control-wrapper">
        <Card>
          <div className="zap-control-header">{search && <Back />}</div>
          ...
          <CardContent>
            <div className="zap-control-input-wrapper">
              <div className="token-select">
                <TokenSelect
                  selected={tokenIn}
                  candidates={tokenInList}
                  handleSelect={handleSelectTokenIn}
                />
                {tokenIn &&
                  tokenIn.address &&
              ...
              <div className="balance">
                ...
                {tokenIn ? (
                  <span className="value">
                    {toDisplayBalanceComma(tokenInBalance, tokenIn.decimals[0])}{" "}
                    {tokenIn.name}
                  </span>
                ) : (
          ...
          <ArrowDown
            className="zap-direction-arrow no-select"
            size={24}
            color={colorWhite}
          />
          ...
          <CardContent>
            <div className="zap-control-input-wrapper">
              <div className="token-select">
                <TokenSelect
                  selected={tokenOut}
                  candidates={tokenOutList}
                  handleSelect={handleSelectTokenOut}
                />
                {tokenOut &&
                  tokenOut.address &&
              ...
              {tradeException === null &&
                !(
                  tokenIn?.name.includes("LP") || tokenOut?.name.includes("LP")
                ) && (
                  <div className="token-input">
                    <TokenInput
                      value={convertedTokenCount}
                      symbol={""}
                      hideSymbol={true}
                      disable
                    >
